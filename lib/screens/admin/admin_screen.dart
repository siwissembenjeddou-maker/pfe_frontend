          ...
          child: Padding(
            ...
            child: SizedBox(
              ...
              child: PieChart(PieChartData(
                sections: [
                  PieChartSectionData(
                      value: users['parent']!.length.toDouble(),
                      title: 'Parents\n${users['parent']!.length}',
                      color: AppTheme.primary,
                      radius: 80),
                  PieChartSectionData(
                      value: users['psychologist']!.length.toDouble(),
                      title: 'Psychs\n${users['psychologist']!.length}',
                      color: AppTheme.accent,
                      radius: 80),
                  PieChartSectionData(
                      value: users['educator']!.length.toDouble(),
                      title: 'Educators\n${users['educator']!.length}',
                ...
                return ListView(
                  ...
                  children: [
                    ...
                    if (roleUsers.isEmpty)
                      EmptyState(icon: Icons.person_outline, message: 'No ${role}s registered'),
    ...
    return Card(
      ...
      child: ListTile(
        leading: CircleAvatar(
          backgroundColor: const Color(0xFF6C3483).withOpacity(0.1),
          child: Text((user['name'] ?? 'U')[0],
              style: const TextStyle(color: Color(0xFF6C3483), fontWeight: FontWeight.bold)),
        ),
...
class _SystemLogTab extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    final logs = List.generate(
        10,
            (i) => {
          'event': ['User login', 'Assessment submitted', 'Review completed', 'User added'][i % 4],
          'time': DateTime.now().subtract(Duration(hours: i * 2)).toString().substring(0, 16),
          'user': 'user_${(i + 1) * 100}',
        });
