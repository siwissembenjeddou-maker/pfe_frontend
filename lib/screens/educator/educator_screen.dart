              ...
              DropdownButtonFormField<String>(
                  ...
                  items: activityTypes
                      .map((a) => DropdownMenuItem(
                          value: a, child: Text('${activityIcons[a]} $a')))
                      .toList(),
          ...
          child: Row(children: [
            Expanded(
                child: Column(
                  ...
                  Text(
                      '${children.length} children · ${today.length} activities today',
    ...
    return Column(children: [
      TableCalendar(
        ...
        calendarStyle: const CalendarStyle(
            selectedDecoration:
                BoxDecoration(color: AppTheme.warning, shape: BoxShape.circle),
            todayDecoration:
...
class _AttendanceTabState extends State<_AttendanceTab> {
  ...
  void initState() {
    super.initState();
    // Initialize all children with default 'present' status
    for (final child in widget.children) {
      _attendance[child.id] = 'present';
    }
  }
          ...
          Row(children: [
            ...
            Expanded(
                child: Column(
                  ...
                  Text(
                      '${child.age} years · ${child.assessments.length} assessments',
                      style: const TextStyle(
          ...
          if (child.assessments.isNotEmpty) ...[
            ...
            Text('Latest: ${child.assessments.last.activityType}',
                style: const TextStyle(
                    color: AppTheme.textSecondary, fontSize: 12)),
            Row(children: [
              ScoreBadge(child.assessments.last.autismScore),
              const SizedBox(width: 8),
              SeverityBadge(child.assessments.last.severityLevel),
            ]),
          ],
        ]),
      ),
      ...
      builder: (_) => AlertDialog(
        ...
        content: SingleChildScrollView(
            child: Column(
                ...
                children: [
              Text(
                  'Current Score: ${report['current_score']?.toStringAsFixed(2) ?? 'N/A'}'),
              ...
              if ((report['trend'] as List?)?.isNotEmpty == true) ...[
                const Text('Trend:',
                    style: TextStyle(fontWeight: FontWeight.bold)),
                ...(report['trend'] as List).map((t) =>
                    Text('${t['date']}: ${t['score']} (${t['activity']})')),
              ],
            ])),
        actions: [
          TextButton(
